...
//		}
	...
	// 이름 입력시 공백 허용 하지 않고, 영문자와 한글만 가능하고록 처리
	private String getName() {
		
		String result = null;
		
		// 사용자에 문자열 받고 -> 예외 발생 여부 확인 -> 예외 발생 -> 예외처리
		outter:while(true) { //outter라벨 붙이기=>break문이 바깥쪽에서 다 벗어남
			//데이터 입력받기
			...
			if(result.length() > 0) {
				...
				//문자열에 영문자와 한글만 포함하고 있는지 체크하는 변수
				boolean check = false;
				...
				for(int i=0; i<result.legnth(); i++) {
					...
					boolean c3 = c>='가' && c<='힣'; //입력문자가 한글 //한글은 다 이어져있음
					
					if(c1 || c2 || c3) {
						// 정상적으로 영문자와 한글이 입력된 경우
				...
				//예외 없이 문자열 체크
				//정상적으로 영문자와 한글이 입력된 경우
				//check=> true : 다시 입력!, false : while(true) 블록을 탈출!
				//result 문자열을 현재 보관
				if(!check) {
					break;
				}
	...
	private String checkPhoneNumber() {
		...
		while (true) {
			...
			if (number.length() > 0) {
				...
				Iterator<Contact> itr = contacts.iterator();
				while (itr.hasNext()) {
					if (itr.next().getPhoneNumber().equals(contacts)) {
...
//		for (int i = 0; i < contacts.size(); i++) {
//			contacts.get(i).printSimpleData();
//			System.out.println("------------------");
//		}
	...
	public void editContact() {
		...
		// 반복을 통해 이름이 같은 Contact 객체를 찾고 -> contactqustndp ckawhrkqtdmf wjwkd
		//반복하면서 분기
		Iterator<Contact> itr = contacts.iterator();
		while(itr.hasNext()) {
			Contact c = itr.next();
		...
		if (contact == null) {
			...
			if (contact instanceof CompanyContact) {
				...
				CompanyContact contactCompany = (CompanyContact) contact;
