...
public class Kiosk_Screen {
	...
	public void step2RecoMenuPrint() {
		...
		if (myBasket.list.isEmpty()) {
			...
			String basketInfo = "┃Sum: ".concat(String.valueOf(totalPrice).concat("| count: ").concat(String.valueOf(totalProduct)));
		...
		System.out.println("\n---------------------------------------");
		System.out.println("[,]: 버거 주문  [.]: 음료 주문  [/]: 사이드 주문");
		System.out.println("---------------------------------------");

		System.out.println("---------------------------------------");
		System.out.println("  주문취소[space]\t\t         주문완료[↵ ]");
		System.out.println("---------------------------------------");
		System.out.println("┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛");
	...
	public void showDrinkByChoice(String drinkType) {
		Connection conn = null;
		try {
			conn = Jdbc_ConPrivider.getConnection();

			List<VO_Sales> list = dao.sendDrinkListByChoice(conn, drinkType);
			for (int i = 0; i < list.size(); i++) {
				System.out.printf("[%d] %s %d￦ %d(kal)\n", i + 1, list.get(i).getProductName(), list.get(i).getPrice(),
			...
			if (nextChoice != -17) {
				// 선택한 상품의 정보 출력
				String type = list.get(nextChoice).getProductName();
				myBasket.list.add(list.get(nextChoice));
				if (type.contains("Burger")) {
					setMenuChoice(drinkType);
				} else {
...
/////////////////////////////////////////////////////////////////////////////////////////////	
	...
	public void sideShow() {
		...
		for (int i = 0; i < sideList.size(); i++) {
			System.out.printf("%d %s %d %d\n", i + 1, sideList.get(i).getProductName(), sideList.get(i).getPrice(),
					sideList.get(i).getCalorie());
		}
	...
	// 선택한 상품의 정보 출력
	public void showSalesBySelected(String s_name, int SaleQty) {

		VO_Sales sales = dao.sendSalesByName(s_name);

		System.out.printf("%s %d %d\n", sales.getProductName(), sales.getPrice(), sales.getCalorie());

		System.out.println("\t-  [".concat(String.valueOf(SaleQty).concat("]".concat("  +"))));
		// + 를 누르면 howMany 갯수 증가
		// - 를 누르면 howMany 갯수 감소
		// 미구현
