...
window.onload = function () {
    ...
    regForm.onsubmit = function () {
        ...
        if (userId.value.trim().length == 0) { //trim은 좌우를 지워줌
            ...
            return false;
        } else if (!(userId.value.trim().length > 2 && userId.value.trim().length < 9)) {//자리수는 정규식 쓰면 간단.일단은 js에 맞춰서 작성
        ...
        if (pw.value.trim().length == 0) {
            ...
            document.querySelector('#msgPw').innerHTML = '비밀번호는 4~10자리까지 가능합니다.';
        ...
        if (userName.value.trim().length == 0) {
            ...
            document.querySelector('#msgName').innerHTML = '이름은 2글자 이상 작성해야 합니다.';
    ...
    var editForm = document.querySelector('#editForm'); //onsubmit
